...
namespace FinalProject.ViewModels
{
    public class EditOrderViewModel : INotifyPropertyChanged
    {
        ...
        public EditOrderViewModel(Order order)
        {
            ...
            // Copy order items to allow adding but not removing
            OrderItems = new ObservableCollection<OrderItem>(order.OrderItems.Select(oi => new OrderItem {
                OrderId = oi.OrderId,
                ItemId = oi.ItemId,
                Quantity = oi.Quantity,
                ItemPrice = oi.ItemPrice,
                Item = oi.Item ?? _dbContext.MenuItems.FirstOrDefault(mi => mi.ItemId == oi.ItemId)
            }));
            SelectedTable = TableList.FirstOrDefault(t => t.TableId == order.TableId);
            SelectedStaff = StaffList.FirstOrDefault(s => s.AccountId == order.AccountId);
        ...
        {
            // Save logic to be handled by parent
        }
